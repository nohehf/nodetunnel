...
use godot::builtin::{
    varray, vdict, Callable, Dictionary, GString, PackedByteArray, PackedStringArray, StringName,
    Variant,
};
use godot::classes::{
    http_client::Method, http_client::Status, HttpClient, Json, Node, Object, WebRtcDataChannel,
    ...
    fn _on_session_description_created(&mut self, type_: GString, sdp: GString) {
        ...
        if self.offer_callback.is_valid() {
            ...
            let _ = self.base_mut().call(
                &call_deferred_name,
                &[wrapper_name.to_variant(), type_var, sdp_var],
            );
        }
    }
...
impl WebRTCClientTransport {
    ...
    pub fn new(
        ...
        godot_print!(
            "[WebRTC] Creating transport with signaling URL: {}",
            signaling_url
        );
    ...
    pub fn handle_session_description(&mut self, type_: GString, sdp: GString) {
        ...
        godot_print!(
            "[WebRTC] Got offer from signal: {} (sdp length: {})",
            type_,
            sdp.len()
        );
    ...
    fn process_signaling(&mut self) -> Result<(), TransportError> {
        ...
        if let Some(offer) = self.pending_offer.take() {
                ...
                .set_local_description(&offer.type_, &offer.sdp)
            {
                Error::OK => {
                    godot_print!(
                        "[WebRTC] Local description set successfully, sending offer to server"
                    );
        ...
        match status {
            ...
            Status::BODY => {
                ...
                godot_print!(
                    "[WebRTC] HTTP response code: {}, body size: {} bytes",
                    response_code,
                    body.len()
                );
                if response_code == 200 {
                    if body.len() > 0 {
                        ...
                        godot_print!(
                            "[WebRTC] Response code 200 but empty body, waiting for more data"
                        );
                    }
                } else {
                    return Err(TransportError::Other(format!(
    ...
    fn send_offer_to_server(&mut self, offer: &SdpDescription) -> Result<(), TransportError> {
        ...
        godot_print!(
            "[WebRTC] Sending offer to signaling server: {} (body size: {} bytes)",
            self.signaling_url,
            json_bytes.len()
        );
        ...
        godot_print!(
            "[WebRTC] HTTP client status before connect: {:?}",
            status_before
        );
    ...
    fn recv(&mut self) -> Result<Vec<ClientEvent>, TransportError> {
        ...
        while self.data_channel.get_available_packet_count() > 0 {
            ...
            if !packet_data.is_empty() {
                godot_print!(
                    "[WebRTC] Received packet: {} bytes, first byte: 0x{:02x}",
                    packet_data.len(),
                    packet_data[0]
                );
            }
