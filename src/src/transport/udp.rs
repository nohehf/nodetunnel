...
impl UDPClientTransport {
    ...
    pub fn send(&mut self, data: Vec<u8>, channel: Channel) -> Result<(), std::io::Error> {
        let packet = match channel {
            Channel::Reliable => self.channel.encode(&data, PacketType::ReliableOrdered),
            Channel::Unreliable => self.channel.encode(&data, PacketType::Unreliable),
        };
...
impl ClientTransport for UDPClientTransport {
    fn send(&mut self, data: Vec<u8>, channel: Channel) -> Result<(), TransportError> {
        self.send(data, channel).map_err(TransportError::from)
    }

    fn recv(&mut self) -> Result<Vec<ClientEvent>, TransportError> {
        Ok(self.recv_packets())
    }
    ...
    fn send_keepalive(&mut self) -> Result<(), TransportError> {
        self.send_keepalive().map_err(TransportError::from)
    }
}
